...
- `module.jai` — Own group-1 params (`MAX_ROUTES`, `MAX_PARAMS`, `MAX_MIDDLEWARE`, `MAX_MOUNTS` — they describe the router, not the core). `#import "http_server"` (default `Handler_Data = void`) + `#import "Basic"`; `#load "router.jai"`.
- `router.jai` — Chi-style router: `#add_context http: *HTTP_Context`, route types (Route, Router, Mount_Point), route registration (get/post/put/http_delete/route), middleware chains (use/proceed), sub-router mounting (mount), path param access (param), and dispatch with per-request context setup. Pattern matching supports literal segments, `:name` param capture, and `*name` wildcard (rest-of-path) segments. The `route_handler` adapter bridges the core's bare `Handler` to `dispatch()` and rides `context.handler_data` with a single `cast(*Router)`; `serve(*Server, *Router)` installs it via the core's `serve`. **Consumers dual-import:** anonymous `#import "http_server"` + `http_router :: #import "http_router"` (named, to namespace `serve`). The lone cast is the bridge slated for removal by re-importing the core with `Handler_Data = Router` (see `docs/plans/2026-06-19-handler-context-refactor-design.md`).
