...
- **NEXT UP (when work resumes) — routing layer: trie path matching + Jai macro helpers.** The current `dispatch` linearly scans routes, calling `match_pattern` per route — O(route count), per the "Routing-overhead A/B" note in Benchmarking (fixed per-request cost ≈ 0; the real cost grows with route count). Planned: follow Go's [chi](https://github.com/go-chi/chi) router design and build a **trie / radix tree** for efficient path matching, plus compile-time **Jai macros** to speed up route registration/dispatch. Related WIP already exists on `origin/routing-module` (router extracted into its own `http_router` module — the cast-free `Handler_Data` refactor) and is a likely starting point.
- **Performance roadmap — closing the gap to the TFB plaintext leaders.** Research into the TechEmpower Round 23 plaintext top 10 (vendored under `research-sources/`; per-candidate reports + synthesis in `docs/research/README.md`) found that all ten leaders (~28M req/s, ~15× our ~1.8M) win the same way: **HTTP/1.1 pipelining with response coalescing** — drain all pipelined requests from one `recv`, append each response into one per-connection buffer, flush with a single `writev` per batch (then reset the Pool per-batch, not per-request). We already have every primitive this needs (zero-copy parser, per-request Pool, single-`writev` send, SO_REUSEPORT shared-nothing workers) — it's a dispatch-loop change, not a rewrite. Prioritized levers, highest first: (1) pipelining + response coalescing; (2) cached `Date` header (1/sec, memcpy); (3) precomputed response prefix via Jai `#run`/`#insert` compile-time specialization (our analog of lithium's template monomorphization); (4) per-worker CPU pinning + `SO_ATTACH_REUSEPORT_CBPF` (`SKF_AD_CPU`) connection steering (the idle-at-peak headroom lever on the Threadripper; the measured libreactor #3-vs-#8 delta); (5) `-march=native` + SIMD parser scans. **Caveat:** pipelining wins *pipelined-plaintext* specifically (real htmx/browser traffic rarely pipelines), so it boosts the leaderboard more than the weather-station target; #2–#5 help every workload. `research-sources/03-libreactor/` is our closest architectural analogue (C epoll reactor).
...
> **UPDATE (2026-06-22):** the O(route-count) linear scan is gone — `dispatch` now walks a **segment trie** (O(path-segments), independent of route count; built by merging per-route partial trees), and `match_pattern` was retired. **Re-measured 2026-06-22** (controlled, same box): route count is free (1 route ≈ 10 routes, ~1.3M at t32/c2000), and the *old* linear-scan master code benches the same as the new trie today — no regression. See the "Multi-path + controlled re-measure" entry below and `docs/plans/2026-06-22-trie-path-matcher-{design,implementation}.md`.
...
- **Route count is free.** 1 route ≈ 10 routes (`hello_world` ≈ multipath `/`-only) — confirms O(path-segments) dispatch; adding routes doesn't slow matching.
- **Path complexity costs only ~4–10%.** multi-path (deep/param/wildcard) vs `/`-only on the *same* 10-route server: within noise at t16, ~4% at t32, ~10% at t8 — the real cost of a longer request line + deeper trie walk + param/wildcard capture, far below the ~36% the old cross-milestone note guessed.
