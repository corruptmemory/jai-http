...
> **UPDATE (2026-06-22):** the O(route-count) linear scan is gone — `dispatch` now walks a **segment trie** (O(path-segments), independent of route count; built by merging per-route partial trees), and `match_pattern` was retired. The many-routes scaling win this predicted is **not yet re-measured** with `wrk` — a benchmark pass is the remaining follow-up; single-route overhead stays ≈0. See `docs/plans/2026-06-22-trie-path-matcher-{design,implementation}.md`.
